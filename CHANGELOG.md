...
#### [Unreleased](https://github.com/Genocs/genocs-library/compare/v8.1.0...HEAD)

- refactor(persistence-efcore)!: Split `Genocs.Persistence.EFCore` into a provider-agnostic core package plus six database provider packages: `Genocs.Persistence.EFCore.SqlServer`, `.PostgreSQL`, `.MySql`, `.Sqlite`, `.Oracle`, and `.MongoDB`. The core package no longer references any database driver. Consumers must install the matching provider package and register it (e.g. `services.AddSqlServerDbProvider()`) alongside `AddEFCorePersistence`; provider selection remains config-driven via `DatabaseOptions:DBProvider` and is dispatched through the new `IEFCoreDbProvider` abstraction resolved from DI.
- refactor(persistence-efcore)!: Extracted Finbuckle multitenancy into the new `Genocs.Persistence.EFCore.MultiTenancy.SqlServer` package. The tenant store (`TenantDbContext`) is now pinned to SQL Server by design. Tenant-scoped database initialization moved from `IDatabaseInitializer` to the new `ITenantDatabaseInitializer` so the core initialization contract stays free of tenant types. The core package no longer references Finbuckle, Mapster-for-tenants, or ASP.NET Core through multitenancy.
- refactor(persistence-efcore): Removed the unused `Serilog.Sinks.MSSqlServer` dependency and the duplicated `UseDatabase`/`DbProviderKeys` code paths from `Genocs.Persistence.EFCore`. Added a `SupportsMigrations` flag on `IEFCoreDbProvider` replacing the hardcoded MongoDB migration check, and made the initialization pipeline (`ApplicationDbInitializer`, `ApplicationDbSeeder`, `CustomSeederRunner`) public for provider and multitenancy packages.
- docs(persistence-efcore): Added agent documentation for the EF Core package family and updated the implementation backlog to reflect the package split.
- refactor(saga-mongodb): Centralized MongoDB client management in `Genocs.Persistence.MongoDB`. `Genocs.Saga.Integrations.MongoDB` now depends on `Genocs.Persistence.MongoDB`, reuses its `MongoOptions` and `IMongoClient`/`IMongoDatabase` registrations, and exposes a parameterless `UseMongoPersistence()` overload for hosts that already configured MongoDB via `AddMongo`. The duplicate `MongoOptions` class was removed and the default configuration section is now `mongoDb`.
- feat(persistence-mongodb): Added public `IServiceCollection.AddMongoClient(MongoOptions)` extension to register the MongoDB client pipeline (singleton `MongoOptions`, `IMongoClient`, `IMongoDatabase`) without pulling in repository, initializer, and seeder registrations. `AddMongo` now delegates to this helper to keep a single source of truth for client setup.
- docs(mongodb): Aligned `Genocs.Persistence.MongoDB` agent documentation with implemented runtime behavior (DI composition, seeding guard semantics, GUID defaults, and encryption surface removal).
- docs(mongodb): Added explicit migration and usage guidance for stabilized MongoDB behavior in package readmes, including serializer compatibility and encryption composition guidance.
- docs(mongodb): Updated MongoDB implementation backlog to complete `MONGO-010` with implementation notes and validation references.
- docs(logging): Added explicit `Genocs.Logging` migration notes for runtime behavior changes introduced by hardening work (`LOGGING-005` to `LOGGING-013`).
- docs(logging): Added release-note checklist items to require migration guidance whenever logging runtime behavior changes.
- docs(logging): Clarified migration actions for sink validation guards, `logger.enabled` semantics, level-switch endpoint contract, exclusion matching, payload capture bounds, and bounded correlation baggage.
- docs(nuget): Standardized package `README_NUGET.md` files across all NuGet packages for clearer, implementation-aligned descriptions and section structure.
- docs(nuget): Added consistent banner image support and unified Support links, including Documentation Portal (`https://genocs-blog.netlify.app/`).
- docs(nuget): Improved `Getting Started` guidance with concrete snippets for core packages (`Genocs.Core`, `Genocs.WebApi`, `Genocs.Auth`, `Genocs.Persistence.MongoDB`, `Genocs.Telemetry`) and harmonized tone/formatting across package readmes.
