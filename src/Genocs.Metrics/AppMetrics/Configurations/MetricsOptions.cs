namespace Genocs.Metrics.AppMetrics.Configurations;

/// <summary>
/// The Metrics settings class.
/// </summary>
public class MetricsOptions
{
    /// <summary>
    /// Default section name.
    /// </summary>
    public const string Position = "metrics";

    /// <summary>
    /// It defines whether the section is enabled or not.
    /// </summary>
    public bool Enabled { get; internal set; }

    /// <summary>
    /// It defines whether the Influx db is enabled or not.
    /// </summary>
    public bool InfluxEnabled { get; internal set; }

    /// <summary>
    /// It defines whether the Prometheus is enabled or not.
    /// </summary>
    public bool PrometheusEnabled { get; internal set; }

    /// <summary>
    /// The Prometheus formatter.
    /// Allowed method are: protobuf or (null).
    /// </summary>
    public string? PrometheusFormatter { get; internal set; }

    /*
    /// <summary>
    /// The InfluxDb url.
    /// </summary>
    public string? InfluxUrl { get; internal set; }

    /// <summary>
    /// The InfluxDb database name.
    /// </summary>
    public string? Database { get; internal set; }
    */

    /// <summary>
    /// The metrics interval.
    /// </summary>
    public int Interval { get; internal set; } = 10;

    /// <summary>
    /// List of tags.
    /// </summary>
    public IDictionary<string, string>? Tags { get; internal set; }
}