...
This folder contains the Genocs apps WebApi host plus a complete BookStore sample implemented with EF Core and SQL Server.
...
- Seed data for a quick end-to-end apps.
...
```bash
dotnet run --project src/apps/integrations/WebApi/Host.csproj
```

Swagger/OpenAPI is enabled by default in this apps host. Check configured routes in `appsettings.json` under `openapi`.
...
dotnet dotnet-ef migrations list \
  --project src/apps/integrations/WebApi/Host.csproj \
  --startup-project src/apps/integrations/WebApi/Host.csproj \
...
dotnet dotnet-ef migrations add <MigrationName> \
  --project src/apps/integrations/Domain.EFCore/Domain.EFCore.csproj \
  --startup-project src/apps/integrations/WebApi/Host.csproj \
  --context BookStoreDbContext \
...
- `src/apps/microservice/WebApi/Genocs.Core.Demo.HelloWorld.http`
...
- See `Extensions/MultiTenancyExtensions.cs` for apps registration logic.
